...
* @Last Modified time: 2020-11-01 12:36:17
*/
...
int main()
{
	...
	if (pid2 == 0)
	{
		...
		// read the processes from the pipe and select only the ones that start with g using the given regex. 
		execlp("grep", "grep", "\\<[g][[:alpha:]]*\\>", NULL);
		close(fd[0]);
		close(fd[1]);
	}
	close(fd[0]);
